...
### Events
@pytest.fixture(params=['GetExifImageData-event-eb.json'])
def event(request):
    '''Return a test event'''
...
@pytest.fixture(params=['GetExifImageData-output.json'])
...
def response_schema():
    ...
    with open(os.path.join(SCHEMA_DIR, 'GetExifLocationDataResponse.schema.json')) as f:
        return json.load(f)
